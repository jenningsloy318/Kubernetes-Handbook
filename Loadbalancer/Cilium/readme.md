...
### Prepare the LB node
1. configure dummy interface(IP, route)
    ...
    nmcli dev set cilium_ext managed yes
...
2. install required package
    ...
    ```
    dnf install conntrack-tools ebtables socat  tc  vim https://www.elrepo.org/elrepo-release-8.el8.elrepo.noarch.rpm
    dnf install  --disablerepo="*" --enablerepo="elrepo-kernel" kernel-ml.x86_64 bpftool.x86_64
    dnf install  https://download.docker.com/linux/centos/7/x86_64/stable/Packages/containerd.io-1.2.6-3.3.el7.x86_64.rpm  docker-ce kubeadm
    ```

3. install cilium, enable `externalIPs` feature
    ```
...
6. add a route on an test node which can access the LB node
